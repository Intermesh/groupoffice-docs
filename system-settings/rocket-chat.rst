...
Since version 6.4.145 GroupOffice can integrate with `Rocket.Chat <https://rocket.chat>`_. With rocket chat you can chat with your team in
...
4. Note the callback URL / or redirect URI on top. For some reason they made it grey so it doesn't stand out. You need that later when adding the client in GroupOffice.
5. Fill in your URL to GroupOffice plus /api/oauth.php. For example: https://groupoffice.example.com/api/oauth.php
...
9. Click save and test if you can login with GroupOffice. You should see a "Login with GroupOffice" button.
...
.. note:: If you get an error double check these settings:
   ...
   - Administration -> Oauth -> Groupoffice: check if id and secret match the parameters in the
     GroupOffice client. Also verify the other fields.